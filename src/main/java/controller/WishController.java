...
//    }
        ...
        protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
            try {
                ...
                resp.getWriter().write(jsonResponse.toString());  // JSON 응답 반환
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
        ...
        protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
            try {
                boolean isWish = Boolean.parseBoolean(req.getParameter("isWish")); // 찜 상태
                int uuid = Integer.parseInt(req.getParameter("uuid")); // 사용자 ID
                int refId = Integer.parseInt(req.getParameter("ref_id")); // 아이템 ID
                String category = req.getParameter("isRentalOrSki"); // 카테고리 (렌탈, 스키 등)
