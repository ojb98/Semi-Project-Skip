...
SELECT 'SKI' AS CATEGORY,
    SR.REVIEW_ID,
    PAYMENT_ID,
    SR.UUID,
    SKI_ID AS REF_ID,
    (SELECT NAME FROM SKI WHERE SKI_ID = SR.SKI_ID) AS REF_NAME,
    RATING,
    REVIEW_COMMENT,
    REVIEW_IMG,
    SR.CREATED_AT,
    REVIEW_RE_ID,
    SRR.UUID R_UUID,
    (SELECT USER_ID FROM USERS WHERE UUID = SRR.UUID) USER_ID,
    CONTENT,
    SRR.CREATED_AT R_CREATED_AT
FROM SKI_REVIEWS SR LEFT JOIN SKI_REVIEW_RE SRR ON SR.REVIEW_ID = SRR.REVIEW_ID
UNION
SELECT 'RENTAL' AS CATEGORY,
    RR.REVIEW_ID,
    PAYMENT_ID,
    RR.UUID,
    RENTALSHOP_ID AS REF_ID,
    (SELECT NAME FROM RENTALSHOP WHERE RENTALSHOP_ID = RR.RENTALSHOP_ID) AS REF_NAME,
    RATING,
    REVIEW_COMMENT,
    REVIEW_IMG,
    RR.CREATED_AT,
    REVIEW_RE_ID,
    RRR.UUID R_UUID,
    (SELECT USER_ID FROM USERS WHERE UUID = RRR.UUID) USER_ID,
    CONTENT,
    RRR.CREATED_AT R_CREATED_AT
FROM RENTALSHOP_REVIEWS RR LEFT JOIN RENTALSHOP_REVIEW_RE RRR ON RR.REVIEW_ID = RRR.REVIEW_ID
UNION
SELECT 'RESORT' AS CATEGORY,
    RR.REVIEW_ID,
    PAYMENT_ID,
    RR.UUID,
    RESORT_ID AS REF_ID,
    (SELECT NAME FROM RESORT WHERE RESORT_ID = RR.RESORT_ID) AS REF_NAME,
    RATING,
    REVIEW_COMMENT,
    REVIEW_IMG,
    RR.CREATED_AT,
