...
//리뷰 수정 팝업
function updatePopup(ski_id) {
	console.log("리뷰 수정 팝업 호출 확인:", ski_id);
	
	const url = `${contextPath}/jsp/update?ski_id=${review_id}`;
	...
	xhr.onload = function() {
		...
		json.list.forEach(function (reviewDto) {
			...
			review_item.innerHTML = `
				...
				<div class="item_ctrl">
					<a href="javascript:updatePopup(${reviewDto.ski_id})">수정</a>
					<a href="${contextPath}/jsp/delete?review_id=${reviewDto.ski_id}" onclick="return confirm('정말 삭제하시겠습니까?');">삭제</a>
				</div>
			`;
