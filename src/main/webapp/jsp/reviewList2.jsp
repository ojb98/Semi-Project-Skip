	...
	function getList(pageNum = 1) {
	    ...
	    xhr.onload = function () {
	        if (xhr.status === 200) {
	            ...
	            const reviews = json.list; // 리뷰 리스트
	            const { pageNum, startPage, endPage, pageCount } = json; // 페이징 정보

	            const reviewList = document.getElementById("reviewList");
	            const pageBtn = document.getElementById("pageBtn");

	            if (!reviewList || !pageBtn) {
	                console.error("reviewList 또는 pageBtn 요소를 찾을 수 없음.");
	                return;
	            }

	            // 기존 목록 및 페이지네이션 삭제 (새로고침 시 중복 방지)
	            ...
	            reviews.forEach(function (review) {
	                ...
	                let imgTag = "";
	                if (review.reviewImg) {
	                    imgTag = `<img src="${contextPath}/reviewImgs/${review.reviewImg}" alt="리뷰 이미지" style="width: 100px; height: 100px;">`;
	                }
	            ...
	            if (pageCount > 1) {
	                for (let i = startPage; i <= endPage; i++) {
	                    ...
	                    pageLink.className = (i === pageNum) ? "active" : "";
	                    pageLink.onclick = function (e) {
	                        e.preventDefault();
	                        getList(i); // 페이지 클릭 시 해당 페이지 불러오기
	                    };
	    ...
	    xhr.onerror = function () {
	        console.error("AJAX 요청 중 네트워크 오류 발생");
	    };

	    xhr.open("GET", `${contextPath}/jsp/reviewList?pageNum=${pageNum}`, true);
	    xhr.send();
	}
	...
	function getList(pageNum) {
		...
		xhr.onload=function() {
			...
			for(let i = children.length-1; i >= 0; i--) {
				reviewList.removeChild(children.item(i));
			}

			
			const resp = xhr.responseText;
			const json = JSON.parse(resp);
			json.list.forEach(function(review){
				const reviewItem = document.createElement("li");
				reviewItem.classList.add('review_item');
				reviewItem.innerHTML +=`리뷰아이디:${review.reviewId} 결제아이디:${review.paymentId} 사용자아이디:${review.uuid} 리조트아이디:${resortId} 별점:${rating} 리뷰내용:${review.resortComment} 리뷰이미지:${review.reviewImg} 작성일:${review.createdAt}`
			})
		}
		xhr.open('get','ResortReviewsCont')
	}
